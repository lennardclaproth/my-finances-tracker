...
func (b *MemoryBus) Subscribe(eventType string, h bus.Handler) (bus.Subscription, error) {
	...
	s := &subscriber{
		...
		eventType: eventType,
		handler:   h,
		queue:     make(chan bus.Envelope, b.queueSize),
	}
...
func (s subscription) Close() error {
	...
	s.bus.mu.Lock()
	m := s.bus.subs[s.sub.eventType]
	if m != nil {
		delete(m, s.sub.id)
		if len(m) == 0 {
			delete(s.bus.subs, s.sub.eventType)
		}
	}
	s.bus.mu.Unlock()
