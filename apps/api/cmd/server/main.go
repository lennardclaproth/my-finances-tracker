...
func run(ctx context.Context, args []string) error {
	...
	jobMgr, importEnqueuer, bulkTagEnqueuer := setupJobs(logger, db, cfg)

	// Wiring: construct handlers and routes at the composition root
	router := setupRouter(logger, db, cfg, importEnqueuer, bulkTagEnqueuer)
...
func setupRouter(
	...
	var transactionRepository = storage.NewSQLXBankTransactionStore(db)
	var importRepository = storage.NewSQLXImportStore(db)
	var vendorRepository = storage.NewSQLXVendorStore(db)
	var providerRepository = storage.NewSQLXProviderStore(db)
	var listingRepository = storage.NewSQLXListingStore(db)
	var dailyRepository = storage.NewSQLXDailyStore(db)
	var diskWriter = storage.NewDisk(cfg.DiskStorage.BasePath + "/import")

	var marketStackClient = marketdata.NewMarketStackClient(
		providerRepository, marketdata.ProviderMarketStack)
	mds := marketdata.NewService(
		listingRepository,
		dailyRepository,
		marketStackClient,
		log)
	...
	router.HandleWithMiddleware(
		"GET /vendors",
		handlers.GetVendors(log, vendorRepository),
		http.WithRequestLogging(log),
	)
	router.HandleWithMiddleware(
	...
	router.HandleWithMiddleware(
		...
		handlers.GetCashflowTransactions(log, transactionRepository),
		http.WithRequestLogging(log),
	)
	router.HandleWithMiddleware(
		"GET /cashflow/analytics/monthly",
		handlers.GetCashflowMonthlyAnalytics(log, transactionRepository),
		http.WithRequestLogging(log),
	)
	router.HandleWithMiddleware(
		"GET /cashflow/analytics/tags",
		handlers.GetCashflowTagDistribution(log, transactionRepository),
		http.WithRequestLogging(log),
	)
	router.HandleWithMiddleware(
	...
	router.HandleWithMiddleware(
		...
		handlers.TagTransactionsBySelection(log, transactionRepository),
		http.WithRequestLogging(log),
	)
	router.HandleWithMiddleware(
		"POST /cashflow/transactions/tag/filter",
		handlers.TagTransactionsByFilter(log, transactionRepository, bulkTagEnqueuer),
		http.WithRequestLogging(log),
	)
	router.HandleWithMiddleware(
		"POST /cashflow/transactions/ignore/selection",
		handlers.IgnoreTransactionsBySelection(log, transactionRepository),
		http.WithRequestLogging(log),
	)
	router.HandleWithMiddleware(
		"POST /cashflow/transactions/ignore/filter",
		handlers.IgnoreTransactionsByFilter(log, transactionRepository),
		http.WithRequestLogging(log),
	)
...
func setupJobs(log logging.Logger, db *storage.DB, cfg *config.Config) (*jobs.Manager, importer.ImportEnqueuer, jobs.BulkTagEnqueuer) {
	...
	importJob := jobs.NewImportJob(
		...
		storage.NewSQLXBankTransactionStore(db),
		storage.NewSQLXPortfolioTransactionStore(db),
